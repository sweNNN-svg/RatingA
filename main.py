...
def tum_animeleri_cek():
    page = 1
    requests_per_second = 1  # İstek hızını azalttık
    ...
    while True:
        ...
        if response.status_code == 200:
            ...
            for anime in animeler:
                # Tüm anime bilgilerini veritabanına ekleyin
                kaydet(anime)
            page += 1
            ...
            time.sleep(10)  # Belirli bir süre bekleyin ve tekrar deneyin
        else:
            print("Hata:", response.text)
            break

        # Her saniyede belirli sayıda istek gönderdikten sonra beklemek
        elapsed_time = time.time() - start_time
        if total_requests % requests_per_second == 0:
            time.sleep(max(0, 1 - elapsed_time))

        # Her dakikada belirli sayıda istek gönderdikten sonra beklemek
        if total_requests % (requests_per_second * seconds_per_minute) == 0:
            time.sleep(60)


def kaydet(anime):
    # Anime bilgilerini veritabanına ekleyin
    insert_query = """
